...
export const clientsService = {
  ...
  // Envia varios clientes pro ERP de uma vez - POST /api/clientes/enviar-erp-em-massa?empresaId=5
  enviarErpBulk: async (
    ids: number[],
    ...
    if (!token) return Promise.reject('Token ausente');
    try {
      const url = `${API_BASE}/api/clientes/enviar-erp-em-massa?empresaId=${encodeURIComponent(empresa.empresa_id)}`;
      ...
      if (!res.ok) {
        let message = 'Falha ao enviar clientes para o ERP';
        try { const err = await res.json(); message = extractErrorMessage(err, message); } catch {}
        return Promise.reject(message);
      }
      return res.json();
